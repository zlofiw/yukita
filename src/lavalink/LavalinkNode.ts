import { AsyncEventBus, YukitaError, YukitaErrorCode, err, ok, toYukitaError, type Result } from '../shared';
      ...
      if (!updateSessionResult.ok) {
        await this.events.emit('error', {
          ...
          error: new YukitaError({
            ...
            message: updateSessionResult.error.message,
            meta: {
              ...(updateSessionResult.error.details ?? {}),
              requestId: updateSessionResult.meta.requestId,
              endpoint: updateSessionResult.meta.endpoint
            }
          })
        });
      }
    ...
    if (!stats.ok) {
      await this.events.emit('error', {
        ...
        error: new YukitaError({
          ...
          meta: {
            ...(stats.error.details ?? {}),
            requestId: stats.meta.requestId,
            endpoint: stats.meta.endpoint
          }
        })
      });
      return;
