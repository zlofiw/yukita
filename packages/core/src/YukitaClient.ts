...
import { mapLoadResult, type LavalinkFilters } from '@yukita/protocol';
import { NodePool } from './NodePool';
import { YukitaPlayer } from './YukitaPlayer';
import type {
  CreatePlayerOptions,
  ExtensionsMap,
...
export class YukitaClient {
  ...
  public async start(): Promise<Result<void>> {
    return this.nodePool.connectAll();
  }
    ...
    try {
      ...
      return err(
        toYukitaError(error, {
          ...
          message: 'Failed to shutdown Yukita client'
        })
      );
    }
  }
  ...
  public async use(plugin: YukitaPlugin): Promise<Result<void>> {
    ...
    if (!satisfies(CORE_VERSION, plugin.compatibleRange, { includePrerelease: true })) {
      return err(
        new YukitaError({
