...
import type { LavalinkFilters, LavalinkPlayerEvent, LavalinkPlayerUpdatePayload } from '@yukita/protocol';
import { PlayerQueue } from './PlayerQueue';
import type { PlaybackTelemetry, PlayerSnapshot, PlayerVoiceState, RepeatMode } from './types';
import type { YukitaClient } from './YukitaClient';
...
export class YukitaPlayer {
  ...
  public async destroy(): Promise<Result<void>> {
    ...
    if (node?.sessionId) {
      ...
      if (!destroyResult.ok) {
        const status = destroyResult.error.meta?.status;
        const isAlreadyAbsent = destroyResult.error.code === YukitaErrorCode.NODE_REST_FAILED
    ...
    if (!result.ok) {
      return err(
        new YukitaError({
          ...
          message: 'Failed to update player on node',
          meta: { contextId: this.contextId, nodeId: this.nodeId },
          cause: result.error
        })
      );
    }
